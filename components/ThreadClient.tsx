...
import { useEffect, useRef, useState, useCallback, FormEvent, useTransition } from "react";
...
export default function ThreadClient({
  ...
  // Quick Reply Popup State (only on thread pages)
  ...
  // Insert quote into Quick Reply popup
  const addQuoteToQr = useCallback((no: string) => {
    if (!threadNo) return;
    setQrOpen(true);
  ...
  // Draggable window logic for QR popup
  ...
  // Event delegation for quotes & post numbers
  useEffect(() => {
    ...
    async function handleClick(e: MouseEvent) {
      ...
      if (qlink) {
        ...
        // If we are on a thread page and the target post is on this page:
        if (threadNo) {
        ...
        // If on board page OR if post is in another thread: open the thread!
        ...
        // Check cache or fetch preview to know the thread number
        if (postCache.current.has(no)) {
      ...
      // 2. Click on post number (.post-no) inside a thread page
      if (threadNo) {
        ...
        if (postNoEl) {
          ...
          e.preventDefault();
          // Open Quick Reply popup and insert quote
          addQuoteToQr(no);
          // Also notify bottom form
  ...
  // File picker handler for Quick Reply
  ...
  return (
    <>
      ...
      {/* Floating Quick Reply Launcher Button (only on thread pages when closed) */}
      {threadNo && !qrOpen && (
        <button
          className="qr-fab"
          onClick={() => {
      ...
      {/* Quick Reply Floating Modal (only on thread pages) */}
      {threadNo && qrOpen && (
        <div
          ref={qrRef}
          ...
          style={
            qrPos
              ? { left: qrPos.x, top: qrPos.y, right: "auto", bottom: "auto" }
              : undefined
          }
        >
            ...
            <form className="qr-form" onSubmit={submitQr}>
              ...
              <div className="qr-actions">
                ...
                <button
                  ...
                  onClick={() => {
                    // Trigger quote event to open the bottom form
                    window.dispatchEvent(new CustomEvent("neochan:quote", { detail: { no: "" } }));
                    setTimeout(() => {
      ...
      {preview && (
        ...
        >
          ...
          {preview.post.image && (
            <img src={preview.post.image.url} alt="" className="pp-thumb" />
          )}
          <div
